...
export class ValidacionProvincia {
  validarCrear(datos: { paisId: string; nombre: string; codigoIso?: string }): void {
    const errores: Errores = {};

    if (!datos.paisId || datos.paisId.trim().length === 0) {
      errores.paisId = "País es requerido";
    }
  ...
  validarActualizar(datos: { paisId?: string; nombre?: string; codigoIso?: string }): void {
    const errores: Errores = {};

    if (datos.paisId !== undefined && datos.paisId.trim().length === 0) {
      errores.paisId = "País no puede estar vacío";
    }
...
export const validacionProvincia = new ValidacionProvincia();
