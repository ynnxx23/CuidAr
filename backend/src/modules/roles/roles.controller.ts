...
export class ControladorRol {
  ...
  static asignarPermiso = asyncHandler(async (req: Request, res: Response) => {
    const resultado = await servicioRol.asignarPermiso(req.params.id as string, req.body.permisoId);
    sendSuccess(res, resultado, "Permiso asignado exitosamente", HTTP_STATUS.CREATED);
  });
  ...
  static removerPermiso = asyncHandler(async (req: Request, res: Response) => {
    ...
    sendSuccess(res, null, "Permiso removido exitosamente");
  });
}
